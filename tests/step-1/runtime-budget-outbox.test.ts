...
test('LLMGateway keeps ingest source budgets and media holds separate', async () => {
  ...
  await assert.rejects(
    () =>
      gateway.withReservation({ scopeType: 'workspace', scopeId: 'ws-main', lane: 'media_safety', day: '2026-06-19', estimateUsd: 0.2 }, async () => 'blocked'),
    /budget exhausted/,
  );
  assert.equal(runtime.active({ workspaceId: 'ws-main', scopeType: 'workspace' }).some((row) => row.mode === 'media_hold'), true);
});
...
test('DataLifecycleService is the bounded cross-owner delete/redaction plane', async () => {
  ...
  `).run();
  store.prepare(`
    INSERT INTO event_journal (name, kind, workspace_id, correlation_id, subject_user_id, privacy_tier, payload_digest, at)
    VALUES ('message.clean', 'event', 'ws-main', 'corr:forget', 'telegram:u1', 'user_payload_digest', 'secret', '2026-06-19T00:00:00.000Z')
  `).run();
  ...
  assert.equal(store.prepare('SELECT payload_digest AS payloadDigest FROM event_journal').get().payloadDigest, '[redacted]');
});
